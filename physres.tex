...
H18 had shown that in the $ne30pg3$ configuration, the column integrated weighted sum of the species, $\langle CLy \rangle$, on day 15 of the terminator test is, to within machine precision, exactly $4\times10^{-6}$ kg kg$^{-1}$. Figure~\ref{fig:terminator} shows this same quantity for the $ne30pg2$ configuration, and $\langle CLy \rangle$ is not everywhere equal to $4\times10^{-6}$ kg kg$^{-1}$, with a minimum value of $3.99936\times10^{-6}$ kg kg$^{-1}$. Linear correlations are not preserved to within machine precision using the coarser resolution physics grid, and have a maximum relative error of $0.016\%$. For comparison, another terminator test is performed with the equivalent grid resolution using the model CAM-SE \citep[$ne30np4$;][]{LetAl2018JAMES}, in which tracers are advected using the spectral element method. The maximum relative error in this configuration is $31.6\%$, three orders of magnitude greater error than the $ne30pg2$ configuration.
