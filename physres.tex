...
This study documents the implementation of a coarser resolution physics grid into the Community Atmosphere Model (CAM), with spectral element dry dynamics and conservative semi-Lagrangian advection of tracers (CAM-SE-CSLAM). The spectral-element and tracer grids are mapped to the coarser finite-volume physics grid, with $\frac{2}{3}$ times fewer degrees of freedom in each horizontal direction, after \cite{HL2018MWR}. Mapping from the coarser physics grid to the higher resolution grids is performed with high-order reconstructions, and a tendency mapping algorithm is developed to ensure shape preservation, linear-correlation preservation and mass conservation. These numerical properties are verified to a high degree of precision through idealized tests.

The coarser resolution physics grid is designed to eliminate grid imprinting that manifests in non-smooth problems using element-based high-order Galerkin methods. The finite-volume physics control volumes encompass a region of the element such that an isotropic representation of the numerics is provided to the physical parameterizations, and it was hypothesized that this method eliminates element boundary noise. Element boundary noise over steep topography is eliminated from the coarser physics grid solution, as shown by a Held-Suarez simulation modified with real-world topography.

The authors tested whether the physics grid aliases the resolved scales of motion to the coarser resolution physics, as suggested by the results of \cite{W1999T}. An exhaustive number of grids were developed and simulated in an aqua-planet configuration, and confirm that the resolved scales of motion are not degraded through the use of a coarser resolution physics grid, and the resolved scales are primarily determined by the dynamics grid resolution ($\Delta x_{dyn}$). This was attributed to two factors; explicit numerical dissipation by the dynamics smoothes out any distinction between the state at the two different grid sizes, and that high-order mapping to the dynamics grids reconstructs scales the physics grid is unable to support.

The coarser physics grid is a factor of $\frac{3}{2} \times \Delta x_{dyn}$, much less then the effective resolution of the model, approximately $5-10 \times \Delta x_{dyn}$. Although the results of \cite{W1999T} suggest the dynamics can be aliased to the coarser resolution physics, Williamson experiments with a physics grid up to a factor $\frac{5}{2} \times \Delta x_{dyn}$. This difference between the physics and dynamics grids may be large enough such that the numerical dissipation in their model can not render these two scales as indistinguishable. That is, our results may show no real difference in the solutions simply because the physics grid does not approach the effective resolution of the model, and that there can be significant savings in computational cost without degrading the resolved scales of motion through the use of a coarser physics grid.
