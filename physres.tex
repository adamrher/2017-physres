...
To understand whether the resolved scales of motion are influenced by the use of a coarser resolution physics grid, a suite of aqua-planet simulations \citep{NH2000ASL,MWO2016JAMES} are carried out over a range of spectral-element grid resolutions, using CAM6 physics ($QPC6$ compset). An aqua-planet is an ocean covered planet in perpetual equinox, with fixed, zonally-symmetric sea surface temperatures idealized after present day Earth. In CAM, their is a strong sensitivity of solutions to the physics time-step \citep[$\Delta t_{phys}$; ][]{WO2003QJR,W2013QJRMS,HR2018JAMES} and it is unclear how $\Delta t_{phys}$ should vary across resolutions.
 ...
 \cite{HR2018JAMES} developed a moist bubble test, which indicate that time-truncation errors are large at high resolution (about $50km$ or less), and may provide incite on a reasonable scaling of $\Delta t_{phys}$ across resolutions. In the moist bubble test a set of non-rotating simulations are performed, in which the grid spacing and bubble radius are simultaneously reduced by the same factor in each run through varying the planetary radius. The test was designed to mimic the reduction in buoyancy length scales that occur when the model resolution is increased in more complex configurations \citep{HETAL2006JCLIM,HR2018JAMES}. 

Here, a scaling for $\Delta t_{phys}$ across resolutions is proposed, based on results of the moist bubble test using CAM-SE-CSLAM coupled to the simple condensation routine of \cite{K1969MM}. The scaling is linear in $\Delta x_{dyn}$ and analogous to a CFL criterion,
\begin{equation}
\Delta t_{phys} = \Delta t_{phys,0} \times \Delta x_{dyn} / \Delta x_{dyn,0}~,\label{eq:dt-scale}
\end{equation}
where $\Delta t_{phys,0}$ is taken to be the standard $1800 s$ used in CAM-SE-CSLAM at low resolution, $\Delta x_{dyn,0} = 111.2km$. Hereafter, $\Delta x_{dyn}$ will be expressed using standard CAM-SE notation, e.g., $\Delta x_{dyn} = 111.2km$ is referred to as $ne30$, which denotes that each face of the cube-sphere grid consists of $30 \times 30$ elements. 

The results of the moist bubble test, displayed as the minimum $\omega$ throughout each one day simulation, is shown in Figure~\ref{fig:bubble} with $\Delta t_{phys}$ determined by equation~\ref{eq:dt-scale}, and an equivalent set of simulations with $\Delta t_{phys} = \Delta t_{phys,0}$ for all resolutions. \cite{HR2018JAMES} has shown that $\omega$ converges to the scaling of,
\begin{equation}
\omega = \omega_0 \times \Delta x_{dyn,0}/\Delta x_{dyn}~,\label{eq:w-scale}
\end{equation}
in the limit of small $\Delta t_{phys}$. Equation~\ref{eq:w-scale} is plotted as grey lines in Figure~\ref{fig:bubble} and is derived from a scale analysis of the Poisson equation \cite{JR2016QJRMS} at hydrostatic scales. The solutions using $\Delta t_{phys}$ from equation~\ref{eq:dt-scale} follow the scaling, whereas fixing $\Delta t_{phys} = 1800s$ across resolutions damps the solution relative to the analytical solution. It should be noted that equation~\ref{eq:w-scale} is only strictly valid for the dry anelastic equations, and that the scaling is maintained when including moist processes is not proven through theory. Regardless, if  
$\Delta t_{phys}$ is too large, the solution has a sizable error.
...
Figure~\ref{fig:bubble} shows the results of the moist bubble test, for a range of spectral element grid resolutions, using both the $pg2$ and $pg3$ grids. The tests were carried out through fixing $\Delta t_{phys}$ across resolutions, and through varying it in proportion to the dynamics grid spacing, $\Delta x_{dyn}$. The magnitude of $\omega$ in the $pg3$ solutions are systematically larger than the $pg2$ solutions, which is primarily a result of the damping effect of integrating the basis functions over a larger control volume.

\section{Results}\label{sec:results}

%A plethora of grids are developed for CAM-SE-CSLAM, and used to understand the sensitivity to physics grid resolution, across a wide range of spectral-element grid resolutions. A hierarchy of idealized model configurations are presented (available from CESM2.0; \url{https://doi.org/10.5065/D67H1H0V}) to illuminate the differences between $pg2$ and $pg3$.
...
%This analysis will make use of an aqua-planet configuration \citep{NH2000ASL,MWO2016JAMES}; an ocean covered planet in perpetual equinox, and fixed, zonally-symmetric sea surface temperatures idealized after present day Earth. The aqua-planets are run for one simulated year, using CAM, version 6 physics (CAM6; $QPC6$ compset in CESM2.0).

\subsubsection{Low Resolution}\label{sec:lowres}

When the physics and dynamics grids are of a different resolution, it is not clear which grid determines the resolved scales of motion. This may be tested through comparing $ne30pg2$, which has a physics grid spacing of approximately $166.8 km$, to a simulation with a $ne20pg3$ grid, in which the physics and the dynamics both have an approximate grid spacing of $166.8 km$. Further, these two solutions are compared against an $ne30pg3$ simulation, which acts as an upper bound on the resolution of $ne30pg2$ solution; the $ne20pg3$ solution then being the lower bound (see Table~\ref{table:grids-lo}). The probability density function (PDF) of upward $\omega$ everywhere in the simulations is shown in Figure~\ref{fig:pgXpanel-lores}b. While in practice the model state `lives' on the physics grid, $\omega$ here it is shown on the $GLL$ grid. It is clear that larger magnitude $\omega$ is more frequent in $ne30pg2$, as compared with $ne20pg3$, and is actually more similar to the $ne30pg3$ distribution.
