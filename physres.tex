...
Experimentation with different physics grid resolutions have so far been limited to models employing the spectral transform method \citep{W1999T,W2014PTRSL}. \cite{LH1997MWR} argue that passing under-resolved states to the physics may be especially problematic in spectral transform models, since the physics are evaluated on a transform grid in grid point space, which contains more degrees of freedom than the spectral representation to prevent aliasing of quadratic quantaties. However, through analyzing an idealized moist baroclinic wave simulation, \cite{LH1997MWR} were not able to attribute any grid-scale noise to the use of the higher-resolution transform grid, which they attribute to the filtering resulting from spectral truncation. After the physics tendencies are transformed into spectral space, it is straightforward to truncate the forcing ay any particular wavelength. \cite{W2014PTRSL} have experimented with different transform grid resolutions, and concluded that the standard higher-resolution quadratic grid actually improves forecast skill over the use of a lower-resolution, linear grid. \cite{W2014PTRSL} suggest that increasing the resolution of the transform grid allows for greater spatial variability, with the spectral truncation mitigtaing any undesireable grid-scale artifacts arising from passing under-resolved states to the physics. 
