...
\subsection{Moist bubble test}\label{sec:bubble}

The results of the previous section are consistent with our hypothesis, that spurious noise is effectively reduced, and visibly eliminated using a $pg2$ grid. We now turn to the question of whether the coarser resolution physics grid has an impact on the resolved scales of motion. Here, the concept of forcing scale is introduced. If one considers a cylinder of height, $H$, diameter, $D$, and constant Archimedean buoyancy, $B_0$, throughout, the vertical velocity scale due to the buoyancy, $W$, may be derived from the Poission equation at hydrostatics scales, $W = \sqrt{B_0H}*H/D$ \citep{JR2016QJRMS,HR2018JAMES}. The buoyancy is determined primarily by the temperature tendencies from the physics (e.g., latent heating due to cloud formation), and hereafter referred to as the {\em{forcing}}. In global atmospheric models, where the unresolved scales are both important and vast, the horizontal forcing scale, or $D$, is related to the resolution the grid is able to support \citep[see Supplementary Figure 1 in][]{HR2018JAMES}.
...
\end{equation}
where $P(\omega_{s})$ is the PDF of the scaled $\omega$, $\omega_{s}$, and $\alpha$ is the ratio of the vertical velocity scale to the vertical velocity scale of the target grid resolution. Using the scale equation for $W$, and assuming that $D$ is linearly related to the grid-spacing, and that $B_0$ and $H$ are unchanged or compensating, $\alpha = \Delta x_{target}/\Delta x$, where $\Delta x$ is the grid spacing and $\Delta x_{target}$ is the grid spacing of the target resolution, taken here to be the grid spacing of the $ne30pg3$ grid. 
