...
Global spectral transform models, while remarkably efficient at small processor counts, do not scale well on massively parallel systems. High-order Galerkin methods are becoming increasingly popular in climate and weather applications due to their high-parallel efficiency, high-processor efficiency, high-order accuracy (for smooth problems), and geometric flexibility facilitating mesh-refinenment applications. High resolution climate simulations with NCAR's Community Atmosphere Model \citep[CAM;][]{CAM5} are typically performed using a continuous Galerkin dynamical core referred to as CAM-SE \citep[CAM Spectral Elements;][]{TES2008JPCS,DetAl2012IJHPCA,LetAl2018JAMES}. CAM-SE may be optionally coupled to a conservative, semi-Lagrangian tracer advection scheme for accelerated multi-tracer transport \citep[CAM-SE-CSLAM;][]{LTOUNGK2017MWR}. Tracer advection then evolves on an entirely separate, finite-volume grid which contains the same degrees of freedom as the spectral element grid.

Element-based Galerkin methods are susceptible to grid-imprinting, and may need be considered when contemplating a particular physics grid. Grid imprinting manifests at the element boundaries, since the global basis is least smooth on the element boundaries \citep[see section {\em{The Quadrature Node Problem}} in][hereafter referred to as HL18]{HL2018MWR}, and the gradients (e.g., the pressure gradient) are systematically tighter. In CAM-SE, the solutions are defined at the quadrature nodes, some of which are located at element boundaries, and the physics tendencies computed from the boundary extrema may exacerbate the grid imprinting. HL18 has shown that through evaluating the physics on the finite-volume tracer advection grid in CAM-SE-CSLAM, element boundary noise is substantially reduced in the model, although still problematic in regions of steep terrain, at low latitudes. They show that through integrating CAM-SE's spectral basis over the control volumes of the finite-volume grid, the volume mean filters out element boundary noise. Additionally, through defining a volume mean state, the finite-volume physics grid is made consistent with assumptions inherent to the physics, and is more appropriate for coupling to other model components (e.g., the land model), which is conventionally performed using a finite-volume based mapping algorithm.

The finite-volume physics grid in HL18 was found by tiling the elements of the cubed-sphere grid with $3\times 3$ equi-angular coordinate lines parallel to the element boundaries, and since the elements are defined by a cubed-sphere grid, the grid approximately isotropic. While a $3\times 3$ physics grid was chosen in order to have the same degrees of freedom as the dynamical core, the control volumes encompass a region of the element in which their proximity to the element boundaries are not equal. Therefore, not every control volume has the same smoothness. This may be avoided through defining a physics grid in which the elements are divided into $2\times 2$ approximiately equal area control volumes, depicted schematically in Figure~\ref{fig:area-schematic}a (hereafter referred to as $pg2$). The control volumes of the $pg2$ grid all have the same proximity to the element boundaries, which is the reason we hypothesize that the $pg2$ grid will have less grid-imprinting than the $pg3$ grid.

Additionally, through integrating CAM-SE's spectral basis over control volumes, a finite-volume physics grid is made consistent with assumptions inherent to the physics, and is more appropriate for coupling to other model components (e.g., the land model), which is conventionally performed using a finite-volume based mapping algorithm.
