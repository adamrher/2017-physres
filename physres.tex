...
When the physics and dynamics grids are of a different resolution, it is not clear which grid determines the resolved scales of motion. This may be tested through comparing $ne30pg2$, which has a physics grid spacing of approximately $166.8 km$, to a simulation with a $ne20pg3$ grid, in which the physics and the dynamics both have an approximate grid spacing of $166.8 km$ (see Table~\ref{table:grids-lo}). The probability density function (PDF) of upward $\omega$ everywhere in the simulations is shown in Figure~\ref{fig:pgXpanel-lores}b. While in practice the model state `lives' on the physics grid, $\omega$ here is shown on the $GLL$ grid. It is clear that larger magnitude $\omega$ is more frequent in $ne30pg2$, as compared with $ne20pg3$, and in fact is remarkably similar to the $ne30pg3$ distribution.

To further understand the changes in vertical motion between the simulations, the concept of forcing scale is introduced. If one considers a cylinder of height, $H$, diameter, $D$, and constant Archimedean buoyancy, $B_0$, throughout, the vertical velocity scale due to the buoyancy, $W$, may be derived from the Poission equation at hydrostatics scales, $W = B_0*H/D$ \citep{HR2018JAMES}. The buoyancy is determined primarily by the temperature tendencies provided by the physics (e.g., latent heating due to cloud formation), and hereafter referred to as the {\em{forcing}}. In global atmospheric models, where the unresolved scales are both important and vast, the horizontal forcing scale, or $D$, is related to the resolution the grid is able to support \citep[see Supplementary Figure 1 in][]{HR2018JAMES}. 

Figure~\ref{fig:pgXpanel-lores}a shows the wave-number power spectrum of the forcing on the $GLL$ grid in the middle-to-upper troposphere, where stratiform heating is common due to detrainment of moisture by the deep convections scheme \citep{ZM1995AO}. The variance is larger for higher wavenumbers in $ne30pg3$ and $ne30pg2$, as compared with $ne20pg3$, supporting the notion that $D$ is larger in $ne20pg3$. The larger $D$ in $ne20pg3$ is consistent with the scale analysis in that the frequency of large magnitude $\omega$ is lower, compared with the $ne30pg2$ and $ne30pg3$ simulations.

The $ne30pg2$ spectra is remarkably similar to the $ne30pg3$ spectra (Figure~\ref{fig:pgXpanel-lores}a), consistent with their similar distribution of large magnitude $\omega$ in Figure~\ref{fig:pgXpanel-lores}b. These figures suggest that $D$ is very similar in $ne30pg2$ and $ne30pg3$. This can be further illustrated through scaling the PDF's,
\begin{equation}
P(\omega_{s}) = \alpha \times P(\omega/\alpha),\label{eq:pdf}
\end{equation}
where $P(\omega_{s})$ is the PDF of the scaled $\omega$, $\omega_{s}$, and $\alpha$ is the ratio of the vertical velocity scale to the vertical velocity scale of the target grid resolution. Using the scale equation for $W$, and assuming that $D$ is linearly related to the grid-spacing, and that $B_0$ and $H$ are unchanged or compensating, $\alpha = \Delta x_{target}/\Delta x$, where $\Delta x$ is the grid spacing and $\Delta x_{target}$ is the grid spacing of the target resolution. Here, $\Delta x_{target}$ refers to the $ne30pg3$ grid. 

If $D$ is in fact determined by the physics grid spacing, $\Delta x_{phys}$, then one sets $\Delta x = \Delta x_{phys}$ in $\alpha$ for the $ne30pg2$ simulation. This scaled PDF, however, severally overestimates the frequency of upward $\omega$ of the target resolution, $ne30pg3$ (Figure~\ref{fig:pgXpanel-lores}c), and it is clear that the similarity of the un-scaled PDF's indicate that $D$ is determined by the dynamics grid spacing, $\Delta x_{dyn}$. The scaled $ne20pg3$ PDF agree's quite well with the $ne30pg3$ distribution, suggesting that the scale parameter $\alpha$ explains the difference in vertical motion between the two simulations.

The authors have determined that their are two reasons $D$ is determined by the dynamics grid, and not the physics grid, in $ne30pg2$. The first reason, is that the hyper-viscosity coefficients are a function of $\Delta x_{dyn}$, and therefore the same in the $pg2$ and $pg3$ simulations. The fourth-order hyperviscosity is rather scale selective, targeting near grid-scale features more so than a second-order operator. The difference in $\Delta x_{phys}$ between $pg2$ and $pg3$ are small enough (a factor of 1.5) that the hyperviscosity renders this distinction somewhat ambiguous. The second reason $D$ is determined by $\Delta x_{dyn}$, is that high-order mapping of the forcing terms from $pg2$, to the higher-resolution $GLL$ and $CSLAM$ grids helps to reconstruct scales that the $pg2$ grid cannot support.

The left panel of Figure~\ref{fig:loworder}a shows a close-up of the wave-number power spectrum of the forcing on the physics grid (dotted) and the $GLL$ grid (solid). In $ne30pg3$ the, variance is similar, and even damped at higher wavenumbers (larger than 65) on the $GLL$ grid relative to the physics grid. Through using high-order mapping in $ne30pg2$, the variance on the $GLL$ grid is actually larger than on the $pg2$ grid, matching the variance of the $ne30pg3$ forcing on the $GLL$ grid, as discussed earlier. Since $\omega$ is determined (albeit diagnostically) from the dynamics, the similar forcing variance on the $GLL$ grid in $ne30pg2$ and $ne30pg3$ is consistent with the similar PDFs of $\omega$ on the $GLL$ grid in the two simulations (see Figure~\ref{fig:loworder}, right panel). Repeating the $ne30pg2$ simulation, but using low-order mapping, i.e., piecewise constant mapping from $pg2$ to $CSLAM$ and bilinear mapping from $pg2$ to $GLL$, the forcing variance on the $GLL$ grid is similar, and even slightly less at high wavenumbers than the $pg2$ grid. Following suit, the frequency of large magnitude $\omega$ on the $GLL$ grid in the low-order run is less compared to the default $ne30pg2$ simulation (Figure~\ref{fig:loworder}, right panel).

Through using the low-order mapping in $ne30pg2$, and by increasing the hyperviscosity coefficients to match the $ne20pg3$ simulation, the simulation more closely resembles the $ne20pg3$ run (Figure~\ref{fig:pgXpanel-lores}). In this case, $D$ is more accurately determined by $\Delta x_{phys}$, since the scaled PDF matches the $ne30pg3$ simulation quite well. Therefore, at low resolution, our default $ne30pg2$ configuration does not indicate that the scales of motion are aliased to resolution of the coarser resolution physics grid, as they more closely resemble the $ne30pg3$ simulation.
